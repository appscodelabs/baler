...
libbuild.REPO_ROOT = expandvars('$GOPATH') + '/src/github.com/appscode/baler'
...
def fmt():
    libbuild.ungroup_go_imports('baler', 'cmd', 'main.go')
    die(call('goimports -w baler cmd main.go'))
    call('gofmt -s -w baler cmd main.go')


def lint():
    call('golint baler cmd min.go')


def vet():
    call('go vet baler cmd')
